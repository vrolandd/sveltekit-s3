...
<div class="mx-auto w-full max-w-7xl px-6 py-10">
	...
	{:else if error}
		<div class="flex flex-col items-center justify-center py-32 text-center">
			<p class="text-lg font-medium text-red-400">Failed to load gallery</p>
			<p class="mt-1 text-sm text-gray-500">{error}</p>
		</div>
	{:else if gallery.length === 0}
		<div class="flex flex-col items-center justify-center py-32 text-center">
			<div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gray-800 text-gray-500">
			...
			<a href="/upload" class="mt-6 rounded-lg bg-indigo-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-400">
				Upload an image
			</a>
		</div>
	{:else}
